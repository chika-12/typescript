...
import { QuestionInput } from '../validators/questions.validator.ts';
import TimeTable from '../models/timeTable.ts';
import { Student } from '../models/studentModel.ts';
import mongoose from 'mongoose';

export const createQuestion = async function (
  data: QuestionInput,
  userId: string,
) {
  ...
  const assignedTeacher = await TeacherAssign.findById(
    examCheck.assignedTeacher,
  );
  if (!assignedTeacher) {
  ...
  if (timeTableExists) {
    throw new AppError(
      'Cannot add questions to exam with scheduled timetable',
      409,
    );
  }
...
export const updateQuestion = async function (
  ...
  if (data.options !== undefined) question.options = data.options as any;
  await question.save();
  return await Question.findById(question._id).populate('exam');
};
...
export const deleteExamQuestion = async function (id: string, userId: string) {
  ...
  const assignedTeacher = (question.exam as any).assignedTeacher;
  if (!assignedTeacher) {
