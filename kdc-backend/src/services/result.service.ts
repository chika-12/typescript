...
import { calculateGrade } from '../utils/calculateGrade.ts';
import mongoose from 'mongoose';

export const createResultWithWrittenScore = async (
  examId: string,
  studentRegNo: string,
  writtenScore: number,
  userId: string,
  ...
  if ((exam.assignedTeacher as any).teacher.toString() !== userId) {
  ...
  if (!student) {
    throw new AppError(
      `$${studentRegNo} is not a valid registration number`,
      403,
    );
  }
  ...
  if (!examSession || examSession.status !== 'submitted') {
    throw new AppError(
      'Student has not completed the CBT for this exam. A Result cannot be created until the CBT is submitted.',
      400,
    );
  }
...
export const getAllResults = async (
  ...
  if (userRole === 'admin' || userRole === 'superAdmin') {
    if ((term && !session) || (!term && session)) {
      throw new AppError(
        'term and session must both be provided together',
        400,
      );
    }
    if (term && session) {
      const termDoc = await Term.findOne({ term, session });
      if (!termDoc) throw new AppError('Term not found for that session', 404);
      const examFilter: any = { term: termDoc._id };
...
export const getResultsById = async (
  ...
  if (userRole === 'student') {
    ...
    const assignedTeacher = (data.exam as any).assignedTeacher as any;
...
export const getBestStudentInSubject = async (subjectId: string) => {
...
export const getBestStudentPerSubjectPerClass = async (
  subjectId: string,
  stdClass: string,
) => {
