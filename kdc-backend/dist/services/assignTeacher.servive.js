...
export const getTeacherAssignments = async (data, query) => {
    let assinged;
...
export const updateTeacherAssignment = async (assignmentId, data) => {
    const assigned = await TeacherAssign.findById(assignmentId);
    if (!assigned) {
        throw new AppError('Assignment not found', 404);
    }
    if (data.className !== undefined)
        assigned.class = data.className;
    ...
    if (data.arm !== undefined)
        assigned.arm = data.arm;
    await assigned.save();
    return assigned;
};
export const deleteTeacherAssignment = async (assignedId) => {
...
//# sourceMappingURL=assignTeacher.servive.js.map